...
import HTTPReceiver, { HTTPReceiverOptions } from "./receivers/HTTPReceiver.ts"
import { OpineReceiverOptions } from "./receivers/OpineReceiver.ts"
import SocketModeReceiver from "./receivers/SocketModeReceiver.ts"
import {
    AckFn,
    AnyMiddlewareArgs,
    BlockAction,
...
export interface ActionConstraints<A extends SlackAction = SlackAction> {
    ...
    callback_id?: Extract<A, { callback_id?: string }> extends any ? string | RegExp
        : never
}
...
export default class App {
    ...
    constructor({
        ...
        this.clientOptions = {
            ...
            slackApiUrl: clientOptions !== undefined
        ...
        if (
            this.developerMode
            && this.installerOptions
            && (typeof this.installerOptions.callbacks === "undefined"
                || (typeof this.installerOptions.callbacks !== "undefined"
                    && typeof this.installerOptions.callbacks.failure === "undefined"))
        ) {
            ...
            this.installerOptions.callbacks = {
                failure: async (
                    ...
                    if (isOpineRequest(req)) {
                        ...
                        await req.respond({
                            ...
                            body: `<html><body><h1>OAuth failed!</h1><div>See stderr for errors.</div></body></html>`,
                        })
                    }
        ...
        if (receiver !== undefined) {
            ...
            if (appToken === undefined) {
                throw new AppInitializationError(
                    "You must provide an appToken when using Socket Mode",
                )
            }
        ...
        if (token !== undefined) {
            ...
            this.logger.error(
                "Never should have reached this point, please report to the team",
            )
            assertNever()
        }
        ...
        if (convoStore !== false) {
            ...
            const store: ConversationStore = convoStore === undefined
                ? new MemoryStore()
                : convoStore
   ...
   */
        ...
        const messageMiddleware = patternsOrMiddleware.map(
            (patternOrMiddleware) => {
                if (
                    ...
                    || patternOrMiddleware instanceof RegExp
                ) {
    ...
    public shortcut<
        ...
        Constraints extends ShortcutConstraints<Shortcut> = ShortcutConstraints<
            Shortcut
        >,
    >(
    ...
    >(
        ...
        const constraints: ShortcutConstraints = typeof callbackIdOrConstraints === "string"
                || callbackIdOrConstraints instanceof RegExp
    ...
    >(
        ...
        const constraints: ActionConstraints = typeof actionIdOrConstraints === "string"
                || actionIdOrConstraints instanceof RegExp
        ...
        const unknownConstraintKeys = Object.keys(constraints).filter(
            (k) =>
                k !== "action_id" && k !== "block_id" && k !== "callback_id"
                && k !== "type",
        )
        if (unknownConstraintKeys.length > 0) {
            this.logger.error(
        ...
        this.listeners.push(
            [onlyActions, matchConstraints(constraints), ...listeners] as Middleware<
                AnyMiddlewareArgs
            >[],
        )
    }
    ...
    public command(
        ...
        this.listeners.push(
            [onlyCommands, matchCommandName(commandName), ...listeners] as Middleware<
                AnyMiddlewareArgs
            >[],
        )
    }
    ...
    public options<Source extends OptionsSource = OptionsSource>(
        ...
        this.listeners.push(
            [onlyOptions, matchConstraints(constraints), ...listeners] as Middleware<
                AnyMiddlewareArgs
            >[],
        )
    }
    ...
    public view<ViewActionType extends SlackViewAction = SlackViewAction>(
        ...
        const constraints: ViewConstraints = typeof callbackIdOrConstraints === "string"
                || callbackIdOrConstraints instanceof RegExp
    ...
    public async processEvent(event: ReceiverEvent): Promise<void> {
        ...
        try {
            if (source.isEnterpriseInstall) {
                authorizeResult = await this.authorize(
                    source as AuthorizeSourceData<true>,
                    bodyArg,
                )
            } else {
                authorizeResult = await this.authorize(
                    source as AuthorizeSourceData<false>,
                    bodyArg,
                )
            }
        } catch (error) {
        ...
        if (body.response_url) {
            listenerArgs.respond = (
                response: string | RespondArguments,
            ): Promise<Response> => {
                const validResponse: RespondArguments = typeof response === "string"
                    ? { text: response }
                    : response

                return fetch(body.response_url, {
                    method: "POST",
...
function buildSource<IsEnterpriseInstall extends boolean>(
    ...
    return {
        ...
        teamId: teamId as IsEnterpriseInstall extends true ? string | undefined
            : string,
        enterpriseId: enterpriseId as IsEnterpriseInstall extends true ? string
            : string | undefined,
...
function isBlockActionOrInteractiveMessageBody(
    ...
    return (body as SlackActionMiddlewareArgs<
        BlockAction | InteractiveMessage

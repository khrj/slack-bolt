import { ActionConstraints, ShortcutConstraints, ViewConstraints } from "../App.ts"
import { ContextMissingPropertyError } from "../errors.ts"
import {
    AnyMiddlewareArgs,
    BlockElementAction,
    DialogSubmitAction,
 ...
 */
export const onlyActions: Middleware<
    AnyMiddlewareArgs & { action?: SlackAction }
> = async ({ action, next }) => {
...
export const onlyShortcuts: Middleware<
    AnyMiddlewareArgs & { shortcut?: SlackShortcut }
> = async ({
    shortcut,
    next,
}) => {
 ...
 */
export const onlyCommands: Middleware<
    AnyMiddlewareArgs & { command?: SlashCommand }
> = async ({ command, next }) => {
 ...
 */
export const onlyOptions: Middleware<
    AnyMiddlewareArgs & { options?: OptionsRequest }
> = async ({ options, next }) => {
 ...
 */
export const onlyEvents: Middleware<
    AnyMiddlewareArgs & { event?: SlackEvent }
> = async ({ event, next }) => {
 ...
 */
export const onlyViewActions: Middleware<
    AnyMiddlewareArgs & { view?: ViewSubmitAction | ViewClosedAction }
> = async ({
    view,
    next,
}) => {
...
export function ignoreSelf(): Middleware<AnyMiddlewareArgs> {
    return async (args) => {
        ...
        const botUserId = args.context.botUserId !== undefined
            ? (args.context.botUserId as string)
            : undefined
...
export function directMention(): Middleware<
...
> {
    return async ({ message, context, next }) => {
        ...
        if (
            matches === null
            // stop when no matches are found
            || matches.index !== 0
            // stop if match isn't at the beginning
...
function isBlockPayload(
    ...
    return (payload as BlockElementAction | OptionsRequest<"block_suggestion">)
        .action_id !== undefined
}

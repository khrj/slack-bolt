...
import { SayFn } from "../utilities.ts"
import { BasicSlackEvent, SlackEvent } from "./base-events.ts"
...
export interface SlackEventMiddlewareArgs<EventType extends string = string> {
    ...
    message: EventType extends "message" ? EventFromType<EventType> : never
 ...
 */
type EventFromType<T extends string> = KnownEventFromType<T> extends never ? BasicSlackEvent<T>
 ...
 */
type WhenEventHasChannelContext<Event, Type> = Event extends { channel: string } | { item: { channel: string } } ? Type
    : never

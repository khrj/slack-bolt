export * from "./block-action.ts"
export * from "./dialog-action.ts"
export * from "./interactive-message.ts"
export * from "./workflow-step-edit.ts"

import { AckFn, RespondFn, SayArguments, SayFn } from "../utilities.ts"
import { BlockAction } from "./block-action.ts"
import { DialogSubmitAction, DialogValidation } from "./dialog-action.ts"
import { InteractiveMessage } from "./interactive-message.ts"
import { WorkflowStepEdit } from "./workflow-step-edit.ts"
 ...
 */
export interface SlackActionMiddlewareArgs<
    Action extends SlackAction = SlackAction,
> {
    payload: Action extends BlockAction<infer ElementAction> ? ElementAction
        : Action extends InteractiveMessage<infer InteractiveAction> ? InteractiveAction
        : Action
    action: Action extends BlockAction<infer ElementAction> ? ElementAction
        : Action extends InteractiveMessage<infer InteractiveAction> ? InteractiveAction
        : Action
    body: Action
 ...
 */
type ActionAckFn<A extends SlackAction> = A extends InteractiveMessage ? AckFn<string | SayArguments>
    : A extends DialogSubmitAction ? AckFn<DialogValidation> // message action and block actions don't accept any value in the ack response
    : AckFn<void>
